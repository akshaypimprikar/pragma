...
- **A third `code-review:code-review` pass found 6 smaller issues in the consolidation above**: an ambiguous qualifier that could skip an unconditional check on non-UI branches, a severity rule that didn't cover Check 1's binary pass/fail output (and didn't carve out Gate 10's expected-output UI-selector-listing command), a missing third verdict state for the disable-model-invocation path, a fallback string that restated canonical wording it was supposed to defer to instead, fallback wording that contradicted its own "don't stall" instruction, and the canonical Note itself omitting that the fallback degrades gracefully rather than halting. Also fixed a sibling copy of the old parallel-execution claim in `plan.md`. All fixed in `parallel-review.md`, `pr-followup.md`, and `plan.md`.
