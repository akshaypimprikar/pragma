...
- If this task's implementation introduces a new protocol declaration, or adds/modifies a mutation on an `@Model` type (the same shapes `/gates` Gate 8 and Gate 10 check for), run a **doubt-driven review** before the GREEN commit: hand a fresh-context subagent only the diff plus this task's acceptance criteria from the plan — not your reasoning or this session's conversation — and instruct it explicitly to find problems with the change or state plainly that it found none. Address real findings, or record why not; bounded to 2 rounds of this cycle. Log any real finding to `.claude/context/incidents.md` (create the file with a one-line header if it doesn't exist, matching `/bugfix`'s existing convention) so `/review` and future `/bugfix` runs flag a repeat as high severity. This exists because `/review` (below) is deliberately not blind to the implementer's reasoning — see `review.md`'s own note on that tradeoff — so nothing else in this pipeline catches a bad call while it's still cheap to reverse.
