...
## v2.0.0 — 2026-09-24
**Features added:** AGENTS.md support and the 16 consumer-facing commands migrated to Agent Skills (`.claude/skills/<name>/SKILL.md`) — breaking change for existing installs; re-running `setup.sh`/`/pragma:init` backs up and removes old `.claude/commands/<name>.md` files. `PreToolUse` guard hook (`scaffold/.claude/hooks/guard_protected_paths.py`, installed via `scripts/install_guard_hook.py`) blocking writes to gate-definition files on `feature/*` branches. `CONSTRAINTS.md` config layer plus Gate 11 gate integrity (`scripts/check_gate_integrity.py`), with check 1 widened to every guarded path. `/review` now re-runs the deterministic gates at the PR HEAD SHA instead of trusting the pasted summary; `/gates` hardening (clean-tree pre-step, wider Gate 0 trigger, Gates 1-2 fail on zero executed tests). Scaffold `gates` CI job. First `claude plugin eval` suite (`evals/`). `/feature` opt-in doubt-driven review. `/pragma-review` moved to `claude-sonnet-5`.
**Key files changed:** `.claude/skills/*/SKILL.md` (migrated from `.claude/commands/`), `scripts/setup.sh`, `.claude/commands/init.md`, `scaffold/.claude/hooks/guard_protected_paths.py` (new), `scripts/install_guard_hook.py` (new), `scripts/check_gate_integrity.py` (new), `CONSTRAINTS.md` (new), `scaffold/.github/workflows/pr-checks.yml`, `evals/` (new), `docs/xcode-27-sdk-migration.md` (new), `README.md`, `.claude-plugin/plugin.json`
**Key architectural decisions:** Instructions layer moved to the cross-tool `AGENTS.md` + SKILL.md open formats (Claude Code remains the only tool pragma is tested in); gate-weakening detection split into a mid-session guard hook plus a diff-based CI check covering the same globs.
