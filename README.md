...
Not a mockup — this is what 75+ merged PRs of `/spec → /plan → /feature → /gates → /review` actually produce. Not released to the App Store (no paid Apple Developer Program membership yet) — the proof is in the merged PR history, not a public release. Full screenshot set in [FinanceTracker's README](https://github.com/akshaypimprikar/financetracker-ios#screenshots).
...
3. **Proven on a real, actively-developed, gitflow-integrated codebase**, not a demo repo — 75+ merged PRs, specs and plans predating every feature, going back to the first commit. Not released to the App Store (no paid Apple Developer Program membership yet) — the proof is the process and the PR history, not a public release. That's still a different claim than "generates a plan.md," and it's checkable: read the actual PR history.
