...
- **Two more copies of the `/pr-followup` staleness bug** (`test.md`'s Trigger section, README.md's `/parallel-review` row) and **two real gaps** (`disable-model-invocation` fallback duplicated independently across 3 files instead of one canonical source — the exact cause of the regression above; `parallel-review.md`'s combined verdict had no severity rule tying `code-review:code-review` findings into the `READY FOR /gates` decision). Consolidated the fallback into `pr-followup.md` alone and added the missing verdict rule. Found by a second `code-review:code-review` pass on the fix PR.
- **A third `code-review:code-review` pass found 5 smaller issues in the consolidation above**: an ambiguous qualifier that could skip an unconditional check on non-UI branches, a severity rule that didn't cover Check 1's binary pass/fail output, a Done-when that lost the disable-model-invocation caveat, a fallback string that restated canonical wording it was supposed to defer to instead, and the canonical Note itself omitting that the fallback degrades gracefully rather than halting. All fixed in `parallel-review.md` and `pr-followup.md`.
